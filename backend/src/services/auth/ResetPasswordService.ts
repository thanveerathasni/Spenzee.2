...
export class ResetPasswordService implements IResetPasswordService {
  ...
  async execute(data: ResetPasswordDto): Promise<void> {
    const resetToken =
      await this._resetPasswordTokenRepository.findByEmail(data.email);
  ...
  private logInvalidResetAttempt(
    email: string,
    reason: string,
  ): void {
    this._logger.warn(LOG_MESSAGES.PASSWORD_RESET_FAILED, {
      email,
      reason,
    });
  }
